// have 5 types, 4 in each (types: fruits, frosting, flavor, garnish, drizzle); items below are placeholders
const items2 = [
  { name: 'Vanilla Frosting', price: 3.5, type: 'frosting', image: 'placeholder.png' },
  { name: 'Chocolate Ganache', price: 4.0, type: 'frosting', image: 'placeholder.png' },
  { name: 'Strawberry Glaze', price: 3.0, type: 'glaze', image: 'placeholder.png' },
  { name: 'Buttercream', price: 4.5, type: 'frosting', image: 'placeholder.png' },
  { name: 'Caramel Drizzle', price: 2.5, type: 'drizzle', image: 'placeholder.png' },
  { name: 'Toasted Almonds', price: 2.8, type: 'topping', image: 'placeholder.png' },
  { name: 'Coconut Flakes', price: 1.8, type: 'topping', image: 'placeholder.png' },
  { name: 'Dark Chocolate Shavings', price: 3.5, type: 'topping', image: 'placeholder.png' },
  { name: 'Lemon Zest', price: 1.2, type: 'garnish', image: 'placeholder.png' },
  { name: 'Mint Leaves', price: 0.8, type: 'garnish', image: 'placeholder.png' },
  { name: 'Whipped Cream', price: 2.0, type: 'topping', image: 'placeholder.png' },
  { name: 'Raspberry Coulis', price: 3.3, type: 'sauce', image: 'placeholder.png' },
  { name: 'Pecan Crumbles', price: 2.7, type: 'topping', image: 'placeholder.png' },
  { name: 'Coffee Cream', price: 4.0, type: 'frosting', image: 'placeholder.png' },
  { name: 'Matcha Powder', price: 2.2, type: 'garnish', image: 'placeholder.png' },
  { name: 'Salted Caramel Chips', price: 3.0, type: 'topping', image: 'placeholder.png' },
]

const items3 = [
  { name: "Chocolate Chips", price: 0.50, type: "chocolate", image: "" },
  { name: "Chopped Nuts", price: 0.80, type: "nuts", image: "" },
  { name: "Whipped Cream", price: 0.50, type: "dairy", image: "" },
  { name: "Caramel Drizzle", price: 0.60, type: "syrup", image: "" },
  
  { name: "Peanut Butter Cups", price: 0.75, type: "candy", image: "" },
  { name: "Coconut Flakes", price: 0.65, type: "nuts", image: "" },
  
  
  
  { name: "Granola", price: 0.70, type: "cereal", image: "" },
  { name: "Cookie Dough Bits", price: 1.00, type: "cookie", image: "" }
];

const items = [
  { name: 'Rainbow Sprinkles', price: 1.5, type: 'sprinkles', image: 'rainbow-sprinkles.png' },
  { name: 'Chocolate Sprinkles', price: 1.5, type: 'sprinkles', image: 'choco-sprinkles.png' },
  { name: "Strawberries", price: 0.90, type: "fruit", image: "chopped-strawberries.png" },
  { name: "Blueberries", price: 0.90, type: "fruit", image: "blueberries.png" },
  { name: "Banana Slices", price: 0.80, type: "fruit", image: "banana-slices.png" },
  { name: "Oreo Crumbs", price: 0.60, type: "cookie", image: "oreo-crumbs.png" },
  { name: "Brownie Chunks", price: 0.95, type: "cookie", image: "brownie-chunks.png" },
  { name: "Gummy Bears", price: 0.70, type: "candy", image: "gummy-bears.png" },
  { name: "Hot Fudge", price: 0.70, type: "syrup", image: "fudge-drizzle.png" },
  { name: "Caramel Drizzle", price: 0.55, type: "syrup", image: "caramel-drizzle.png" },
  { name: "Marshmallows", price: 0.60, type: "candy", image: "marshmallow.png" },
  { name: "M&M’s", price: 0.70, type: "candy", image: "m&m.png" },
  { name: "Almond", price: 0.85, type: "nuts", image: "almonds.png" },
  
]

export { items }
